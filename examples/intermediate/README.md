...
- `03-priority-queue`: A heap-backed priority queue example.
- `04-factory-pattern`: A contract that deploys instances of another contract.
- `05-token-wrapper`: A contract that wraps a standard token to add new functionality.
- `06-upgradable-proxy`: A basic proxy pattern for contract upgradability.
